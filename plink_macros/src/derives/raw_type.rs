...
fn fn_read(output: &mut String, parsed: &Struct) {
    ...
    for field in &parsed.fields {
        if field.attrs.iter().any(|attr| attr == "pointer_size") {
            output.push_str(&format!("{}: <{} as RawTypeAsPointerSize>::read(cursor)?,", field.name, field.ty));
        } else {
            output.push_str(&format!("{}: <{} as RawType>::read(cursor)?,", field.name, field.ty));
        }
    }
    output.push_str("    })\n");
    output.push_str("}\n");
